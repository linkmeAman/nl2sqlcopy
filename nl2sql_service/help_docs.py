...
ROUTE_DOCS: tuple[RouteDoc, ...] = (
    RouteDoc(
        ...
        slug="health",
        title="Health Check",
        summary="Returns service liveness and current database connectivity.",
        description="Use this endpoint to confirm that the API process is running and to see whether PostgreSQL is connected, unreachable, or unavailable.",
        response_example={"status": "ok", "db": "connected"},
        error_cases=("Normally returns HTTP 200 even when the DB is degraded.",),
        related=("/telemetry/summary", "/telemetry/recent"),
    ),
    RouteDoc(
        method="GET",
    ...
    RouteDoc(
        ...
        description="Use this endpoint when app or backend callers want one blocking JSON response containing the final answer and SQL metadata.",
        request_example={"query": "show me the 5 most recent inquiries", "top_k": 3, "request_id": "demo-001"},
    ...
    RouteDoc(
        ...
        description="Use this endpoint for terminal and debugging workflows where progress events are useful before the final answer response.",
        request_example={"query": "show me the 5 most recent inquiries", "top_k": 3, "request_id": "demo-001"},
